...
train_df = pd.DataFrame([{'path': c_path,
              ...
              for c_path in glob('C:\input\\train\*\*')+glob('C:\input\\additional\*\*')])
...
test_df = pd.DataFrame([dict(path = c_path,
                           image_name = os.path.basename(c_path))
              for c_path in glob('C:\input\\test\*')])
...
auto_classifier = TPOTClassifier(generations=5, population_size=5, verbosity=2)
